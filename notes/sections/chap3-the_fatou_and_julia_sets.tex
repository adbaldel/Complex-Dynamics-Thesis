...
\begin{remark}[$\sim$ is an equivalence relation]
    ...
    transitive for \eqref{eq:equivalence_relation} and $g^p(y) = g^q(z)$ imply
    that
    \begin{equation*}
        g^{n+p}(x) = g^{m+q}(z);
    \end{equation*}
...
\begin{remark}[Uniform convergence $\implies$ pointwise convergence]
    ...
    Rational Functions (\ref{sec:a_topology_on_the_rational_functions}), we
