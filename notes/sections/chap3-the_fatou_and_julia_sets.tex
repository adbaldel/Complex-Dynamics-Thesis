...
$\epsilon_0 = \delta$ between $w$ and $z$ so that their $n$-th iterates are as
close as we want, see Figure~\ref{fig:equicontinuity_for_iterates}.
\begin{figure}
    \centering
    \resizebox{\textwidth}{!}{\input{../figures/tikz/equicontinuity_for_iterates}}
...
\begin{proposition}[Finite number of Fatou components $\implies$ they are periodic]
...
\begin{theorem}[$\{f: D \to \ExtC - \{0, 1, \infty\} \mid f$ analytic$\}$ is normal]
...
\begin{theorem}[$\{f: D \to \ExtC - \{a_f, b_f, c_f\} \mid f$ analytic$\}$ is normal]
...
\begin{theorem}[$\{f: D \to \ExtC \mid f$ analytic and $f(z) \neq \varphi_j(z)$ for $j=1$, $2$, $3\}$ is normal]
