...
\begin{proof}
    ...
    implies that $w$ is exceptional for $R$. Now such a map need not be
    conjugate to a polynomial (consider, for example, $z \mapsto z^{-2}$) but
    from Theorem~\ref{thm:rational_maps_have_at_most_two_exceptional_points},
    $R^2$ must be. Thus, we have proved the theorem.
\end{proof}
...
the Julia set can be non-empty, and our first task will be to settle this issue.
The case when the rational map $R$ has degree one is trivial and of little
interest, but in all other cases, $J$ is not empty.
...
\begin{proof}
    ...
    (Theorem~\ref{thm:complete_invariance_is_preserved_under_complement_interior_and_boundary}).
...
In the next chapter (Chapter~\ref{chp:the_structure_of_the_Fatou_set}) we shall
prove a stronger, but similar, result for the components of $J$
(Theorem~\ref{thm:J_disconnected_implies_uncountably_many_components}).
...
\begin{theorem}[$W$ open and $W \cap J \neq \emptyset \implies \bigcup R^n(W)$ explodes to $\ExtC - E(R)$ and $R^n(W)$ converges to a set that contains $J$]
    \label{thm:union_of_iterates_of_an_open_set_that_meet_J_explode_to_ExtC_minus_exceptional_points}
...
\begin{theorem}[$J \subset \closure{\{\text{periodic points}\}}$]
    \label{thm:J_contained_in_closure_of_periodic_points_set}
...
\begin{remark}[$z$ non-exceptional point $\implies J \subset \closure{[z]}$] We
    now consider the relationship between $J$ and an orbit $[z]$ of a
...
\begin{remark}[$z$ non-exceptional point and $z \in J \implies J =
    \closure{[z]}$] If $z$ is in $J$, then it is not exceptional, so $[z]$, and
    hence also $\closure{[z]}$, is contained in $J$: thus \emph{if $z$ is in
    $J$, then $J = \closure{[z]}$}. 
\end{remark}
...
\begin{remark}[Using a computer to illustrate $J$]
    ...
    speaking, one selects a non-exceptional $z$ and then computes successive
...
\begin{remark}[Notation $R^{-n}(E) \to J$]
    ...
    - \{\zeta\}$ has the required property. In
    Chapters~\ref{chp:forward_invariant_components} and
    \ref{chp:the_no_wandering_domains_theorem}, we shall analyze completely the
    possible forward images of $\{R^n(z) \mid n \geq 1\}$ of a point $z$ in $F$
    and after that, Theorem~\ref{thm:R_inverse_n_E_converges_to_J} will be even
    more significant.
\end{remark}
...
\begin{example}
    ...
    for the convenience of some readers, we give an expanded version in the
...
\begin{definition}[Pre-periodic point]
    \label{def:preperiodic_point}
    A point $\zeta$ is \emph{pre-periodic} under $R$ if it is not periodic, but
    a forward image of it is.
\end{definition}
