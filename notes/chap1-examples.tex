...
\begin{definition}[Forward and Backward Invariant]
    Given a set $C \subseteq \ExtC$ and a rational function $R: \ExtC \to
    \ExtC$, we say $C$ is:
    \begin{enumerate}
        \item \emph{Forward invariant} if $R(C) \subseteq C$.
        \item \emph{Backward invariant} if $R^{-1}(C) \subseteq C$.
    \end{enumerate}
\end{definition}
...
and
\[
    \sum_{0 \le i < j \le n}^{n} r_i r_j = \frac{a_{n-2}}{a_n}, \quad \sum_{0 \le i < j < k \le n}^{n} r_i r_j r_k = \frac{a_{n-3}}{a_n}, \quad \cdots
\]
...
\]
where $g = \frac{1}{z}$. See figure~\ref{fig:action_of_z_plus_one_over_z_near_infinity} for the action of $R$ near $\infty$.
