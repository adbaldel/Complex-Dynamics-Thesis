...
estudia la Dinámica Compleja antes de formalizar sus conceptos centrales.
...
\begin{definicion}[Transformación de Möbius]
    ...
    \] 
    donde $z$, $a$, $b$, $c$, $d$ son números complejos. Seguiremos los
    convenios habituales con el punto del infinito en la esfera de Riemann,
    $\ExtC$: $R(\infty)=a/c$ y $R(-c/d) = \infty$.
\end{definicion}
...
\begin{definicion}[Punto fijo, periódico y preperiódico]
    Sea $R \colon \ExtC \to \ExtC$ una función racional. Un punto $z_0 \in
    \ExtC$ diremos que es:
    \begin{itemize}
        \item un \emph{punto fijo} de $R$ si $R(z_0) = z_0$;
        \item un \emph{punto periódico de periodo $p \geq 1$} de $R$ si
        $R^p(z_0) = z_0$ y $R^n(z_0) \neq z_0$ para todo $n < p$;
        \item o un \emph{punto preperiódico} de $R$ si $z_0$ no es periódico,
        pero para algún $k \geq 1$, $R^k(z_0)$ es periódico.
    \end{itemize}
\end{definicion}

La preguntas fundamentales que nos hacemos son: si iteramos $R$ partiendo de un
punto arbitrario $z_0 \in \ExtC$, ¿cómo se comportará la secuencia $R^n(z_0)$?;
y si tomamos otro punto $w_0 \in \ExtC$ cercano a $z_0$, ¿$R^n(w_0)$ se
comportará de manera similar a $R^n(z_0)$? A lo largo de todo el trabajo se
utilizará $R^n$ para referirse a componer $R$ consigo misma $n$ veces.

\begin{definicion}[Órbita]
    Sea $R \colon \ExtC \to \ExtC$ una función racional y $z_0 \in \ExtC$.
    Llamamos \emph{órbita} u \emph{órbita positiva} de $z_0$ al conjunto
    \[
        \mathcal{O}(z_0) = \mathcal{O}^+(z_0) = \{z_n = R^n(z_0) \mid n \in \Nzero\}.
    \]
    Y \emph{órbita negativa} de $z_0$ al conjunto
    \[
        \mathcal{O}^-(z_0) = \{z \mid R^n(z) = z_0, \text{para algún $n \in \Nzero$}\} = \bigcup_{n \in \Nzero}R^{-n}(z_0).
    \]
\end{definicion}

En este momento resulta interesante introducir el concepto de
\emph{conjugación}. No solo nos va a servir para simplificar el análisis de las
transformaciones de Möbius, sino que también nos ayudará a encontrar familias de
funciones con el mismo comportamiento dinámico por ser una relación de
equivalencia.
...
En el caso de la ecuación~\ref{eq:mobius_ej1}, utilizando $g = 1/(z-\zeta)$ para
trasladar el punto fijo $\zeta$ al infinito, encontramos que $R$ es conjugada a
$S(z) = 2 + z$. $S$ es una traslación y $S^n(z) = 2n + z$. Observamos que el
único punto fijo de $S$ es el infinito (que se corresponde con el punto fijo
$\zeta = 1$ de $R$) y que dado cualquier punto $z_0 \in \ExtC$, $S^n(z_0)$ tiene
al punto fijo cuando $n \to \infty$. Luego, $\lim_{n \to \infty} R^n(z_0) =
...
\]
la cual posee dos puntos fijos, $\zeta_1 = 1$ y $\zeta_2 = -1$. Esta vez usamos
$g(z) = (z-\zeta_2)/(z-\zeta_1)$ para enviar los puntos fijos $1$ y $-1$ al
$\infty$ y al $0$ respectivamente. Obtenemos que $R$ es conjugada a la rotación
$S(z) = iz$. Como $(-i)^4=1$, se tiene que $S^4(z)=z$ para todo $z$ y, en
consecuencia por las propiedades de la conjugación, $R^4(z)= z$. Bajo esta
transformación, todas las órbitas son periódicas (de período divisor de 4) y,
salvo en los puntos fijos, ninguna órbita converge.
...
En las transformaciones de Möbius, ya tengan uno o dos puntos fijos, todos los
puntos describen órbitas similares. En particular, dados dos puntos cercanos
cualesquiera $z_0$ y $w_0 \in \ExtC$, ambos describirán órbitas prácticamente
iguales.
